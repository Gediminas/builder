...
function GetBuildCommands($build_info, $rebuild, $log_file)
{
	...
	$cmds = Array();
	$cmds[0] = "%COMSPEC% /c .\\CMake\\bin\\cmake \"$dir\" -G \"Visual Studio 15 2017 Win64\" 2>&1";
	
	//Platform is not empty only for wxwidgets 64bit build.
	if( !empty($platform) ) {
		$cmds[1] = "%COMSPEC% /c \"\"{$exe}\" \"{$sln}\" /m /v:m /t:{$build} /p:PlatformToolset=v141 /p:Configuration=\"{$conf}\" /p:Platform=\"{$platform}\"";// > \"{$log_file}\"";
	}
	else {
		$cmds[1] = "%COMSPEC% /c \"\"{$exe}\" \"{$sln}\" /m /v:m /t:{$build} /p:PlatformToolset=v141 /p:Configuration=\"{$conf}\"";// > \"{$log_file}\"";
	}
	return $cmds;
}
