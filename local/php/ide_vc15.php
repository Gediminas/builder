...
//***************************************************
// Build *.sln using Microsoft Visual Studio C++ 15.0 with compiler 14.0
//***************************************************
...
function GetBuildCommands($build_info, $rebuild, $log_file)
{
	...
	if( !empty($platform) ) {
		$cmds[0] = "%COMSPEC% /c \"\"{$exe}\" \"{$sln}\" /m /v:m /t:{$build} /p:PlatformToolset=v140 /p:Configuration=\"{$conf}\" /p:Platform=\"{$platform}\"";// > \"{$log_file}\"";
	}
	else {
		$cmds[0] = "%COMSPEC% /c \"\"{$exe}\" \"{$sln}\" /m /v:m /t:{$build} /p:PlatformToolset=v140 /p:Configuration=\"{$conf}\"";// > \"{$log_file}\"";
	}
	return $cmds;
}
