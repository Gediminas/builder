...
function bugs_no_estimation_notify($product_name, $milestone, $supervisors, $week_day, $do_log = false)
{
	...
	foreach ($bugs_by_user as $mail_to => $user_bugs)
	{
		...
		$user_name_ascii      = iconv("UTF-8", "ASCII//TRANSLIT", $user_name);
		...
		$super_subject = "$product_name_mile: [{$user_no_set_cnt}] {$user_name_ascii} has not estimated bug(s)";
		...
		foreach ($user_bugs as $bug)
		{
			...
			$body .= "	<tr>
						...
						<td> {$bug->m_severity} </td>
						<td> {$bug->m_priority} </td>
						<td> {$bug->m_status} </td>
						<td> {$bug->m_summary} </td>
						<td> {$bug->m_reporter->m_real_name} </td>
						</tr>";

			//echo " #" . $bug->m_bug_id . " - " . $bug->m_status . " - " . $bug->m_summary . "\r\n";;
			echo " #{$bug->m_bug_id} - {$bug->m_severity}/{$bug->m_priority}/{$bug->m_status} - {$bug->m_summary} ({$bug->m_reporter->m_real_name})\r\n";;
		}
