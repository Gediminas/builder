...
class CGeneratePage
{
	...
	private function GenerateBody()
	{
		...
		echo "<body=on_load>\n";
			echo "<table border=0>\n";
				...
				echo "<col width=300 />";
					echo "<tr>\n";
						...
						{
							echo "<td width=400>\n";
								echo "<font color='#448800'> $host ($ip)</font>";
								...
								$daemon_start_diff = (0 != $daemon_start) ? format_time(strtotime($curr_time) - strtotime($daemon_start)) : "not started";
								$night_build_diff  = (0 != $daemon_start) ? format_time(strtotime($curr_time) - strtotime($night_build)) . " ago" :  "never";

								echo "    <table border=0>\n";
									echo "       <tr> <td> <font size='-2' style='color:gray'>  server runs: </font> </td> <td> <font size='-2' style='color:gray'>  $daemon_start_diff </font></td></tr>\n";
									echo "       <tr> <td> <font size='-2' style='color:gray'>  night build: </font> </td> <td> <font size='-2' style='color:gray'>  $night_build_diff </font></td></tr>\n";
								echo "    </table>";
			...
			echo "</table>\n";
			$val = explode(".", $txt);
			foreach ($val as $ii => $v) $val[$ii] = intval($v);
	...
	private function GenerateModulesPanel()
	{
		...
		$all      = isset($_GET['all'])      ? $_GET['all']      : 0; // this is main module!!! has to be removed
		$bcomment = isset($bcomment) ? $bcomment : (isset($_GET['bcomment']) ? $_GET['bcomment'] : false); // this is main module!!! has to be removed
		echo "<INPUT type='hidden' name='all'      value='" . $all                    . "'>\n"; // this is main module!!! has to be removed
		...
		foreach ($modules_sorted as $module => $prio)
		{
			...
			$name_txt = is_file("$mod_conf") ? file("$mod_conf") : NULL;
			$var = 0;
			for ($i=5; 0<=$i; $i--) if (is_file($module . '/fnc'. $i .'.ccx')) include($module . '/fnc'. $i .'.ccx');
...
$bcomment = isset($bcomment) ? $bcomment : (isset($_GET['bcomment']) ? $_GET['bcomment'] : false);
?>